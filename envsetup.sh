...
EOF
    ...
    local A=""
    local i
    for i in `cat $T/build/envsetup.sh $T/vendor/deluxe/build/envsetup.sh | sed -n "/^[[:blank:]]*function /s/function \([a-z_]*\).*/\1/p" | sort | uniq`; do
      A="$A $i"
    done
    echo $A
}
...
function build_build_var_cache()
{
    ...
    cached_vars=(`cat $T/build/envsetup.sh $T/vendor/deluxe/build/envsetup.sh | tr '()' '  ' | awk '{for(i=1;i<=NF;i++) if($i~/get_build_var/) print $(i+1)}' | sort -u | tr '\n' ' '`)
    cached_abs_vars=(`cat $T/build/envsetup.sh $T/vendor/deluxe/build/envsetup.sh | tr '()' '  ' | awk '{for(i=1;i<=NF;i++) if($i~/get_abs_build_var/) print $(i+1)}' | sort -u | tr '\n' ' '`)
...
function check_product()
{
    ...
    if (echo -n $1 | grep -q -e "^deluxe_") ; then
        DELUXE_BUILD=$(echo -n $1 | sed -e 's/^deluxe_//g')
	export BUILD_NUMBER=$( (date +%s%N ; echo $DELUXE_BUILD; hostname) | openssl sha1 | sed -e 's/.*=//g; s/ //g' | cut -c1-10 )
    else
        DELUXE_BUILD=
    fi
    export DELUXE_BUILD
...
function lunch()
{
    ...
    if [ $? -ne 0 ]
    then
        # if we can't find a product, try to grab it off the deluxeOS-Devices GitHub
        T=$(gettop)
        cd $T > /dev/null
        vendor/deluxe/build/tools/roomservice.py $product
        cd - > /dev/null
        check_product $product
    else
        T=$(gettop)
        cd $T > /dev/null
        vendor/deluxe/build/tools/roomservice.py $product true
        cd - > /dev/null
    fi
...
function cmka() {
    if [ ! -z "$1" ]; then
        for i in "$@"; do
            case $i in
                bacon|deluxe|otapackage|systemimage)
                    mka installclean
                    mka $i
                    ;;
                *)
...
. $ANDROID_BUILD_TOP/vendor/deluxe/build/envsetup.sh
